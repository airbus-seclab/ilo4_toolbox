...
with open(outdir + "/bootblock.sig", "wb") as fff:
    fff.write(img0.to_str(img0.signature))
...
with open(outdir + "/bootstrap.sig", "wb") as fff:
    fff.write(img1.to_str(img1.signature))
...
for i, mod in enumerate(mod_list):
    print "    %2x) %30s, type 0x%02x, size 0x%08x, crc 0x%08x" % (i, mod.module, mod.type, mod.decompressed_size, mod.img_crc)
